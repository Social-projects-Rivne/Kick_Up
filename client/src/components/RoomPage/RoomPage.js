...
import {
  AppBar, Tabs, Tab, Typography, Grid, Avatar, Card, CardActions, CardContent, CardMedia,
  CardActionArea, Button, ListItemText, ListItem, ListItemAvatar, Badge, Fab, Paper
              ...
              </Fab>
              {
                roomPagePosts.map((post, itr) =>
                  <Grid key={itr} item xs={12} className="room-details-card-grid">
                    <PostCard
                      data={post} currentUser={this.props.user ? this.props.user.id : null}
          ...
          {(value === 4 && <TabContainer>
            <Grid container spacing={24} className="room-details-card">
              ...
              {roomPagePosts
                ...
                .map((post, itr) =>
                  <Grid key={itr} item xs={12} className="room-details-card-grid">
                    <PostCard data={post} currentUser={this.props.user ? this.props.user.id : null}
                              roomId={this.props.match.params.id}/>
                  </Grid>
                )}
            </Grid>
                ...
                <Grid item lg={3} md={4} sm={6} xs={12}>
                  <Link component={RouterLink} to={`/profile/` + member.id} className="room-page-member-link">
                    <ListItem className="avatar-center">
                      ...
                      <ListItemText primary={(member.first_name || 'Shy') + ' ' + (member.last_name || 'Unicorn')}
